    ...
    <div className="p-4 md:p-8 space-y-6 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p className="text-muted-foreground">Real-time telemetry and status.</p>
        </div>
        <div className="flex items-center gap-3">
             ...
             <div className={cn("px-3 py-1 rounded-full text-xs font-medium border flex items-center gap-2", 
                ...
                {pumpConnected ? "Pumps Online" : "Pumps Offline"}
             </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="bg-card/50 backdrop-blur-sm">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Spectrometer</CardTitle>
                <Activity className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
                <div className="text-2xl font-bold">{currentVoltage.toFixed(3)} V</div>
                <p className="text-xs text-muted-foreground">Real-time voltage</p>
            </CardContent>
        </Card>
        ...
        <Card className="bg-card/50 backdrop-blur-sm">
            ...
            <CardContent>
                <div className="text-2xl font-bold">{missionStatus}</div>
                <p className="text-xs text-muted-foreground">{automationRunning ? "Automation Running" : "System Idle"}</p>
            </CardContent>
        </Card>
        ...
        <Card className="bg-card/50 backdrop-blur-sm lg:col-span-2">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                 <CardTitle className="text-sm font-medium">Pump Angles</CardTitle>
            </CardHeader>
            <CardContent className="grid grid-cols-4 gap-2">
                {Object.entries(pumpAngles).map(([axis, angle]) => (
                    <div key={axis} className="text-center">
                        <div className="text-xs text-muted-foreground font-bold mb-1">{axis}-Axis</div>
      ...
      <Card className="col-span-4 h-[500px] flex flex-col">
         <CardHeader>
            <CardTitle>PID Error Tracking</CardTitle>
            <p className="text-sm text-muted-foreground">Offset visualization (Baseline: X=0, Y=2, Z=4, A=6)</p>
         </CardHeader>
                ...
                <LineChart data={pidData}>
                    ...
                    <Tooltip 
                        ...
                        formatter={(value: any, name: any, props: any) => {
                             ...
                             return [rawVal ? rawVal.toFixed(4) : (typeof value === 'number' ? value.toFixed(4) : value), `Error ${name}`]
                        }}
                    />
