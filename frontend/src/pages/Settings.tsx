        ...
        <CardContent className="space-y-5 sm:space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4 xl:grid-cols-3">
              ...
              <div className="space-y-2"><Label>原始帧上行频率 (Hz，最低 20)</Label><Input type="number" min="20" max="200" value={hw.publish_rate} onChange={e => setHw(p => ({ ...p, publish_rate: parseInt(e.target.value) || 90 }))} /></div>
              <div className="space-y-2"><Label>聚合输出频率 (Hz)</Label><Input type="number" min="1" max="50" value={hw.spectro_output_hz} onChange={e => setHw(p => ({ ...p, spectro_output_hz: parseInt(e.target.value) || 10 }))} /></div>
            ...
            <p className="text-xs text-muted-foreground">原始帧会按至少 20 Hz 保存到后端；聚合输出频率只影响实时展示和任务摘要的负载。</p>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4">
