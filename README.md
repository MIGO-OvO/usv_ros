...
- 分光计：`spectrometer_auto_start` 等
- 动作按钮：`刷新设备`、`测试泵控连接`、`仅保存`、`保存并应用`
...
- `POST /api/hardware/apply`：在保存配置后触发 `/usv/pump_reconnect`。
...
3. **无分光数据**
   检查 ESP32 连通性，以及分光模块工作状态。

4. **保存并应用失败**
   检查 `/usv/pump_reconnect` 是否存在，确认节点仍在运行，并查看返回信息。
